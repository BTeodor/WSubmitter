...
if(!is_file('../library/config.php')) die('Check the type in config file and make sure the ddlsub folder is in the main folder');
require_once('../library/config.php');
$conn=mysql_connect($config['db']['host'], $config['db']['username'],$config['db']['password']);
mysql_select_db($config['db']['dbname']) or die ("Unable to connect to MySQL");
$tableprefix='xf_';
...
switch($act) {
	...
	case 'sonec': {
		...
		$cats=implode(',',$cats);
		$query = mysql_query("SELECT `node_id`, `title` FROM ".$tableprefix."node where `node_id` in (".$cats.") and node_type_id='Forum'") or die(mysql_error());
		...
		{
			echo "<option value = '$result[node_id]'>$result[title]</option>";
		}
	...
	case 'getposts': {
		...
		if($ws_config['latestposts']!='') {
			...
			$sub_latest = " AND post_date > '".$last."'"; 
		}
		else $sub_latest='';
		if(isset($_REQUEST['page']) && intval($_REQUEST['page']) > 0) $ppage = intval($_REQUEST['page']); else $ppage = 1;
		$offset = ($ppage - 1) * $limit;
		if($submitted_ids!='') $sub_not_in="  AND `thread_id` NOT in (".$submitted_ids.")"; else $sub_not_in='';
		if($ws_config['debug']) { printr("<b>sub_not_in</b>"); printr($sub_not_in); }
		$query = mysql_query("SELECT COUNT(`thread_id`) as num FROM ".$tableprefix."thread WHERE `node_id` = '".mysql_real_escape_string($cid)."'".$sub_latest.$sub_not_in."") or die('err:'.mysql_error());
		...
		$qu = mysql_query("SELECT `thread_id`, title FROM ".$tableprefix."thread WHERE `node_id` = '".mysql_real_escape_string($cid)."'".$sub_latest.$sub_not_in."ORDER BY `thread_id` DESC $offset") or die(mysql_error());
		...
		while($re = mysql_fetch_assoc($qu))
		{
			...
			$purl=$ws_config['urlf'].'index.php?threads/'.$re['thread_id'].'/';
			$ptitle = clean($re['title']);
			echo '<tr id="row'.$re['thread_id'].'" tid="'.$re['thread_id'].'" ttitle="'.$ptitle.'" turl="'.$purl.'" ttype="'.$ptype.'""><td width="3%" style="border: 2px solid #7bc4df;">'.$i.'</td>';
		...
		while($nrs<$limit) {
			...
			foreach($cats as $cat) {
				if(count($postsids)>0) {
					...
					$notin=" and thread_id not in (".$inpostsids.")";
				}
				...
				$check=mysql_query("SELECT `thread_id` FROM `".$tableprefix."thread` WHERE `node_id` = '".mysql_real_escape_string($cat['id'])."'".$sub_latest.$sub_not_in.$notin."") or die(mysql_error());
				if(mysql_num_rows($check)==0) if(!in_array($cat['id'],$catstoskip)) $catstoskip[]=$cat['id'];
				$qu = mysql_query("SELECT `thread_id`, title FROM `".$tableprefix."thread` WHERE `node_id` = '".mysql_real_escape_string($cat['id'])."'".$sub_latest.$sub_not_in.$notin." ORDER BY `thread_id` DESC LIMIT $lim") or die(mysql_error());
				while($re = mysql_fetch_assoc($qu))
					{
						foreach($ws_config['cats'] as $ccat) if($ccat['id']==$cat['id']) { $ptype = $ccat['type']; break; }
						$postsids[]=$re['thread_id'];
						$purl=$ws_config['urlf'].'index.php?threads/'.$re['thread_id'].'/';
						$pid = $re['thread_id'];
						$ptitle = clean($re['title']);
