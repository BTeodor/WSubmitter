...
mysql_select_db($INFO['sql_database']) or die ("Unable to connect to MySQL");
if($INFO['sql_tbl_prefix']!='') $tableprefix=$INFO['sql_tbl_prefix']; else $tableprefix='';
...
switch($act) {
	...
	case 'sonec': {
		...
		$cats=implode(',',$cats);
		$query = mysql_query("SELECT `name`, `id` FROM ".$tableprefix."forums where `id` in (".$cats.")") or die(mysql_error());
	...
	case 'getposts': {
		...
		if($ws_config['latestposts']!='') {
			...
			$sub_latest = " AND start_date > '".$last."'"; 
		}
		else $sub_latest='';
		...
		$query = mysql_query("SELECT `tid` FROM ".$tableprefix."topics WHERE approved='1' and `forum_id` = '".mysql_real_escape_string($cid)."'".$sub_latest.$sub_not_in."") or die('err:'.mysql_error());
		...
		$qu = mysql_query("SELECT `tid`, `title`, `title_seo`  FROM ".$tableprefix."topics WHERE approved='1' and `forum_id` = '".mysql_real_escape_string($cid)."'".$sub_latest.$sub_not_in."ORDER BY `tid` DESC $offset") or die(mysql_error());
		...
		while($re = mysql_fetch_assoc($qu))
		{
			...
			$purl=$ws_config['urlf'].'index.php?/topic/'.$re['tid'].'-'.$re['title_seo'].'/';
	...
	case 'sallc': {
		...
		while($nrs<$limit) {
			...
			foreach($cats as $cat) {
				...
				$check=mysql_query("SELECT `tid` FROM `".$tableprefix."topics` WHERE approved='1' and `forum_id` = '".mysql_real_escape_string($cat['id'])."'".$sub_latest.$sub_not_in.$notin."") or die(mysql_error());
				if(mysql_num_rows($check)==0) if(!in_array($cat['id'],$catstoskip)) $catstoskip[]=$cat['id'];
				$qu = mysql_query("SELECT `tid` , `title` , `title_seo` FROM `".$tableprefix."topics` WHERE approved='1' and `forum_id` = '".mysql_real_escape_string($cat['id'])."'".$sub_latest.$sub_not_in.$notin." ORDER BY `tid` DESC LIMIT $lim") or die(mysql_error());
				while($re = mysql_fetch_assoc($qu))
					{
