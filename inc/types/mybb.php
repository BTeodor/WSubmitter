...
if(!is_file('../inc/config.php')) die('Check the type in config file and make sure the ddlsub folder is in the main folder');
require_once('../inc/config.php');
$conn=mysql_connect($config['database']['hostname'], $config['database']['username'],$config['database']['password']);
mysql_select_db($config['database']['database']) or die ("Unable to connect to MySQL");
if($config['database']['table_prefix']!='') $tableprefix=$config['database']['table_prefix']; else $tableprefix='';
...
switch($act) {
	...
	case 'sonec': {
		...
		$cats=implode(',',$cats);
		$query = mysql_query("SELECT `name`, `fid` FROM ".$tableprefix."forums where `fid` in (".$cats.")") or die(mysql_error());
	...
	case 'getposts': {
		...
		if($submitted_ids!='') $sub_not_in="  AND `tid` NOT in (".$submitted_ids.")"; else $sub_not_in='';
		if($ws_config['debug']) { printr("<b>sub_not_in</b>"); printr($sub_not_in); }
		$query = mysql_query("SELECT COUNT(`tid`) as num FROM ".$tableprefix."threads WHERE `fid` = '".mysql_real_escape_string($cid)."'".$sub_latest.$sub_not_in."") or die('err:'.mysql_error());
		...
		$qu = mysql_query("SELECT `tid`, `subject` FROM ".$tableprefix."threads WHERE `fid` = '".mysql_real_escape_string($cid)."'".$sub_latest.$sub_not_in."ORDER BY `tid` DESC $offset") or die(mysql_error());
		...
		while($re = mysql_fetch_assoc($qu))
		{
			...
			$purl=$ws_config['urlf'].'showthread.php?tid='.$re['tid'];
			$ptitle = clean($re['subject']);
			echo '<tr id="row'.$re['tid'].'" tid="'.$re['tid'].'" ttitle="'.$ptitle.'" turl="'.$purl.'" ttype="'.$ptype.'""><td width="3%" style="border: 2px solid #7bc4df;">'.$i.'</td>';
		...
		while($nrs<$limit) {
			...
			foreach($cats as $cat) {
				if(count($postsids)>0) {
					...
					$notin=" and tid not in (".$inpostsids.")";
				}
				...
				$check=mysql_query("SELECT `tid` FROM `".$tableprefix."threads` WHERE `fid` = '".mysql_real_escape_string($cat['id'])."'".$sub_latest.$sub_not_in.$notin."") or die(mysql_error());
				if(mysql_num_rows($check)==0) if(!in_array($cat['id'],$catstoskip)) $catstoskip[]=$cat['id'];
				$qu = mysql_query("SELECT `tid` , `subject` FROM `".$tableprefix."threads` WHERE `fid` = '".mysql_real_escape_string($cat['id'])."'".$sub_latest.$sub_not_in.$notin." ORDER BY `tid` DESC LIMIT $lim") or die(mysql_error());
				while($re = mysql_fetch_assoc($qu))
					{
