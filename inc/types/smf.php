...
if(!is_file('../Settings.php')) die('Check the type in config file and make sure the ddlsub folder is in the main folder');
require_once('../Settings.php');
$conn=mysql_connect($db_server, $db_user,$db_passwd);
mysql_select_db($db_name) or die ("Unable to connect to MySQL");
if($db_prefix!='') $tableprefix=$db_prefix; else $tableprefix='';
...
switch($act) {
	...
	case 'sonec': {
		...
		$cats=implode(',',$cats);
		$query = mysql_query("SELECT `id_board`, `name` FROM ".$tableprefix."boards where `id_board` in (".$cats.")") or die(mysql_error());
		...
		{
			echo "<option value = '$result[id_board]'>$result[name]</option>";
		}
	...
	case 'getposts': {
		...
		if($submitted_ids!='') $sub_not_in="  AND `id_topic` NOT in (".$submitted_ids.")"; else $sub_not_in='';
		if($ws_config['debug']) { printr("<b>sub_not_in</b>"); printr($sub_not_in); }
		$query = mysql_query("SELECT COUNT(`id_topic`) as num FROM ".$tableprefix."topics WHERE `id_board` = '".mysql_real_escape_string($cid)."'".$sub_not_in."") or die('err:'.mysql_error());
		...
		$qu = mysql_query("SELECT `id_topic`, id_first_msg FROM ".$tableprefix."topics WHERE `id_board` = '".mysql_real_escape_string($cid)."'".$sub_not_in."ORDER BY `id_topic` DESC $offset") or die(mysql_error());
		...
		while($re = mysql_fetch_assoc($qu))
		{
			...
			$purl=$ws_config['urlf'].'index.php?topic='.$re['id_topic'];
			$sq=mysql_fetch_assoc(mysql_query("select subject from ".$tableprefix."messages where id_msg = '".$re['id_first_msg']."'"));
			$ptitle = clean($sq['subject']);
			echo '<tr id="row'.$re['id_topic'].'" tid="'.$re['id_topic'].'" ttitle="'.$ptitle.'" turl="'.$purl.'" ttype="'.$ptype.'""><td width="3%" style="border: 2px solid #7bc4df;">'.$i.'</td>';
		...
		while($nrs<$limit) {
			...
			foreach($cats as $cat) {
				if(count($postsids)>0) {
					...
					$notin=" and id_topic not in (".$inpostsids.")";
				}
				...
				$check=mysql_query("SELECT `id_topic` FROM `".$tableprefix."topics` WHERE `id_board` = '".mysql_real_escape_string($cat['id'])."'".$sub_not_in."".$notin."") or die(mysql_error());
				if(mysql_num_rows($check)==0) if(!in_array($cat['id'],$catstoskip)) $catstoskip[]=$cat['id'];
				$qu = mysql_query("SELECT `id_topic`, id_first_msg FROM `".$tableprefix."topics` WHERE `id_board` = '".mysql_real_escape_string($cat['id'])."'".$sub_not_in."".$notin." ORDER BY `id_topic` DESC LIMIT $lim") or die(mysql_error());
				while($re = mysql_fetch_assoc($qu))
					{
						foreach($ws_config['cats'] as $ccat) if($ccat['id']==$cat['id']) { $ptype = $ccat['type']; break; }
						$postsids[]=$re['id_topic'];
						$purl=$ws_config['urlf'].'index.php?topic='.$re['id_topic'];
						$sq=mysql_fetch_assoc(mysql_query("select subject from ".$tableprefix."messages where id_msg = '".$re['id_first_msg']."'"));
						$ptitle = clean($sq['subject']);
						$pid = $re['id_topic'];
