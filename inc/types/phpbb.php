...
if(!is_file('../config.php')) die('Check the type in config file and make sure the ddlsub folder is in the main folder');
require_once('../config.php');
$conn=mysql_connect($dbhost, $dbuser,$dbpasswd);
mysql_select_db($dbname) or die ("Unable to connect to MySQL");
if($table_prefix!='') $tableprefix=$table_prefix; else $tableprefix='';
...
switch($act) {
	...
	case 'sonec': {
		...
		$cats=implode(',',$cats);
		$query = mysql_query("SELECT `forum_name`, `forum_id` FROM ".$tableprefix."forums where `forum_id` in (".$cats.")") or die(mysql_error());
		...
		{
			echo "<option value = '$result[forum_id]'>$result[forum_name]</option>";
		}
	...
	case 'getposts': {
		...
		if($ws_config['latestposts']!='') {
			...
			$sub_latest = " AND topic_time > '".$last."'"; 
		}
		else $sub_latest='';
		if(isset($_REQUEST['page']) && intval($_REQUEST['page']) > 0) $ppage = intval($_REQUEST['page']); else $ppage = 1;
		$offset = ($ppage - 1) * $limit;
		if($submitted_ids!='') $sub_not_in="  AND `topic_id` NOT in (".$submitted_ids.")"; else $sub_not_in='';
		if($ws_config['debug']) { printr("<b>sub_not_in</b>"); printr($sub_not_in); }
		$query = mysql_query("SELECT COUNT(`topic_id`) as num FROM ".$tableprefix."topics WHERE `forum_id` = '".mysql_real_escape_string($cid)."'".$sub_latest.$sub_not_in."") or die('err:'.mysql_error());
		...
		$qu = mysql_query("SELECT `topic_id`, `topic_title` FROM ".$tableprefix."topics WHERE `forum_id` = '".mysql_real_escape_string($cid)."'".$sub_latest.$sub_not_in."ORDER BY `topic_id` DESC $offset") or die(mysql_error());
		...
		while($re = mysql_fetch_assoc($qu))
		{
			...
			$purl=$ws_config['urlf'].'viewtopic.php?t='.$re['topic_id'];
			$ptitle = clean($re['topic_title']);
			echo '<tr id="row'.$re['topic_id'].'" tid="'.$re['topic_id'].'" ttitle="'.$ptitle.'" turl="'.$purl.'" ttype="'.$ptype.'""><td width="3%" style="border: 2px solid #7bc4df;">'.$i.'</td>';
		...
		while($nrs<$limit) {
			...
			foreach($cats as $cat) {
				if(count($postsids)>0) {
					...
					$notin=" and topic_id not in (".$inpostsids.")";
				}
				...
				$check=mysql_query("SELECT `topic_id` FROM `".$tableprefix."topics` WHERE `forum_id` = '".mysql_real_escape_string($cat['id'])."'".$sub_latest.$sub_not_in.$notin."") or die(mysql_error());
				if(mysql_num_rows($check)==0) if(!in_array($cat['id'],$catstoskip)) $catstoskip[]=$cat['id'];
				$qu = mysql_query("SELECT `topic_id` , `topic_title` FROM `".$tableprefix."topics` WHERE `forum_id` = '".mysql_real_escape_string($cat['id'])."'".$sub_latest.$sub_not_in.$notin." ORDER BY `topic_id` DESC LIMIT $lim") or die(mysql_error());
				while($re = mysql_fetch_assoc($qu))
					{
						foreach($ws_config['cats'] as $ccat) if($ccat['id']==$cat['id']) { $ptype = $ccat['type']; break; }
						$postsids[]=$re['topic_id'];
						$purl=$ws_config['urlf'].'viewtopic.php?tid='.$re['topic_id'];
						$pid = $re['topic_id'];
						$ptitle = clean($re['topic_title']);
